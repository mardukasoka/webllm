...
function summarizePreparedCandidate(item) {
  if (!item?.ok) {
    return {
      ok: false,
      model: item?.model || "unknown",
      status: item?.status || "invalid-candidate",
      error: item?.error || "Candidate failed validation."
    };
  }
  ...
  return {
    ...
    proposal: {
      ...
      confidence: envelope.proposal.confidence,
      changes: envelope.proposal.changes.map((change) => ({
        path: change.path,
        reason: change.reason
      })),
      testsRecommended: [...envelope.proposal.testsRecommended],
...
export function compareFederatedCouncilCandidates({ reviewPacket, candidates } = {}) {
  ...
  const candidateSummaries = prepared.map(summarizePreparedCandidate);
