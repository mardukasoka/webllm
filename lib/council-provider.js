...
export async function generateCouncilParticipant({
  ...
  if (participant.runtime === "anthropic") {
    if (!participant.modelInstance) {
      throw new Error(
        `Anthropic participant "${participant.name || participant.id}" is not configured.`
      );
    }

    const result = await generateAnthropicAssistant({
  ...
  if (participant.runtime === "gemini") {
    if (!participant.modelInstance) {
      throw new Error(
        `Gemini participant "${participant.name || participant.id}" is not configured.`
      );
    }
